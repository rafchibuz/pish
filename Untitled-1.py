...
class CarAnimation:
    def __init__(self, csv_file):
        ...
        required_columns = ['shirota', 'dolgota', 'speed_kmh']
    ...
    def get_can_data(self, point_index):
        ...
        current_speed = float(self.route_data['speed_kmh'].iloc[point_index])
        ...
        # Расход топлива (рассчитываем на основе скорости)
        if current_speed <= 0:
    ...
    def create_data_panel(self, can_data):
        ...
        if len(time_str.split(':')) == 2:  # Если только минуты:секунды
        ...
        data_items = [
            ...
            ('⚡ СКОРОСТЬ', f"{can_data['speed']} км/ч"),
            ('⛽ РАСХОД', f"{can_data['fuel']} л/100км")
        ]
        ...
        for label, value in data_items:
            ...
            # Значение
            self.ax_data.text(0.05, y_pos - 0.05, value, color='#3498DB', 
                             fontsize=16, fontweight='bold', transform=self.ax_data.transAxes)
            y_pos -= line_height
        
        # Добавляем информацию о данных из CSV
        self.ax_data.text(0.05, 0.05, f"📊 Данные из CSV: {len(self.route_data)} точек", 
                         color='#95A5A6', fontsize=10, transform=self.ax_data.transAxes)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("2. Что в CSV есть колонки: 'shirota', 'dolgota', 'speed_kmh'")
